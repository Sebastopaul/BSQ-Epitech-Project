...
{
    char **arr = malloc(sizeof(int) * (size[1] + 2) * size[0]);

    for (int row = 0; row < size[0]; row++)
        arr[row] = malloc(sizeof(char) * size[1] + 2);
    return (arr);
}
void print_map(char **map, int size[2], square_t square)
{
    int x = square.x - square.size + 1;
    int y = square.y - square.size + 1;
    int s_size = square.size;
